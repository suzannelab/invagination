...
def delamination(sheet, manager, face_id,
                 contract_rate=2,
                 critical_area=1e-2,
                 radial_tension=1.,
                 nb_iteration=0,
...
def type1_transition(sheet, manager, face_id,
                     critical_length=0.3, geom=SheetGeometry):
...
def face_elimination(sheet, manager, face_id,
                     geom=SheetGeometry):
...
def check_tri_faces(sheet, manager):
    ...
    tri_faces = sheet.face_df[(sheet.face_df['num_sides'] < 4) & (
        sheet.face_df['is_mesoderm'] is False)]['id']
    manager.extend(
        [(face_elimination, f,
...
def check_enter_in_process(sheet, manager,
                           mesoderm,
